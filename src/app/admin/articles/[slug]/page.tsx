...
export default function EditArticlePage() {
  ...
  const [existingDir, setExistingDir] = useState("blog");
  ...
  useEffect(() => {
    fetch("/api/admin/articles/" + slug)
      .then((r) => r.json())
      .then((data) => {
        if (data.error) { alert("Article not found"); router.push("/admin/articles"); return; }
        setTitle(data.title || "");
        ...
        setImageCreditUrl(data.imageCreditUrl || "");
        setFeatured(data.featured || false);
        setStatus(data.status || "published");
        setContent(data.content || "");
        setFaq(data.faq || []);
        setExistingDir(data.dir || "blog");
        setLoading(false);
      })
      .catch(() => { alert("Failed to load article"); router.push("/admin/articles"); });
  }, [slug, router]);

  function addFaq() { setFaq([...faq, { question: "", answer: "" }]); }
  ...
  function updateFaq(index: number, field: keyof FaqItem, value: string) {
    const updated = [...faq]; updated[index][field] = value; setFaq(updated);
  }
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        reader.onload = () => {
          ...
          uploadToGithub(generateSlug() + "-" + Date.now() + "." + ext, base64);
        };
  ...
  async function uploadToGithub(filename: string, base64: string) {
    try {
      ...
      if (res.ok) { const data = await res.json(); setImage(data.url); setShowImageUpload(false); }
    ...
    try {
      const res = await fetch("/api/admin/articles/" + slug, {
        ...
        body: JSON.stringify({
          ...
          featured, status: publishStatus, content, dir: existingDir,
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch("/api/admin/articles/" + slug + "?dir=" + existingDir, { method: "DELETE" });
      if (res.ok) { router.push("/admin/articles"); }
          ...
          <div className="rounded-xl border border-[#1E293B] bg-[#0B1020] p-4 space-y-4">
            ...
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={featured} onChange={(e) => setFeatured(e.target.checked)}
                className="h-4 w-4 rounded border-[#1E293B] bg-[#080B14] text-[#0060E0]" />
              ...
              <div className="flex gap-3">
                <label className="flex items-center gap-2">
                  <input type="radio" name="status" checked={status === "draft"} onChange={() => setStatus("draft")}
                    className="h-4 w-4 border-[#1E293B] bg-[#080B14] text-[#0060E0]" />

...
export async function generateStaticParams() {
  return getAllPosts("ai-tools").map((post) => ({ slug: post.slug }));
}
...
export async function generateMetadata({
  ...
  const post = getPost("ai-tools", slug);
  if (!post) return {};
...
export default async function AiToolPostPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const related = getPostsByCategory("ai-tools", post.category)
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <article className="mx-auto max-w-3xl px-0 sm:px-0">
          ...
          <header className="mb-8">
            ...
            <div className="mt-4">
              <ShareButtons title={post.title} url={`/ai-tools/${slug}`} description={post.description} />
            </div>
          </header>
